...
/*
 在执行　avformat_open_input(...)　前，设置ffmpeg的日志回调函数　av_log_set_callback(..)，然后在回调函数里面获取的信息就是摄像头设备了。

 要注意的是，这种方法非线程安全，如果同时有其它的日志输出，就会影响判断。

 但正常使用的话，不存在这种问题，如果是在程序初始化时使用，那就OK。
 */
int test()
{
	...
	err_code = avformat_open_input(&pFormatCtx,"video=USB 视频设备",ifmt,NULL);
	if( err_code!=0 ){
